...
export function deleteCookie(name: string, options = {} as any) {
  options.expires = new Date(0);
  setCookie(name, "", options);
}

export const calculateScrollWidth = (columns: any, minusWidth: number) => {
  const windW = window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth;

  const windownTableWidth = windW - minusWidth;
  ...
  return Math.max(columnsWitdth, windownTableWidth);
};

export function addCountryCode(phoneNumber: string, phoneCode: number) {
